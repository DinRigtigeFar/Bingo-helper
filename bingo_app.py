...
def btn_click(number):
    ...
    # Store the popped numbers in a message. Only numbers removed from our cards are stored here.
    message = txt_to_card.pop_list(number, my_cards)
    # Now we 'disassemble' the BingoCardDict object into a dict and override the session with the updated cards
    session['my_cards'] = [i.card for i in my_cards]

    return render_template('number_grid.html', message=message, sesh=current_line, drawn_numbers=session.get('drawn_numbers'))
